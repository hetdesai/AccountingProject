    ...
    Private Sub fillsale1()
        Try
            ...
            While dr.Read
                ...
                dg1.Item(16, i).Value = dr.Item("srno")
                i = i + 1
            End While
            dr.Close()
            close1()
...
label:
                        ...
                        Try
                            ...
                            dr = cmd.ExecuteReader
                            While dr.Read
                                dg1.Item(4, dg1.CurrentCell.RowIndex).Value = dr.Item(0)
                                dg1.Item(6, dg1.CurrentCell.RowIndex).Value = dr.Item(1)
                            End While
                            dr.Close()
                            close1()
            ...
            If e.ColumnIndex = 14 Then
                dg1.Item(dg1.ColumnCount - 2, e.RowIndex).Value = Format(Val(Math.Round((dg1.Item(dg1.ColumnCount - 3, e.RowIndex).Value) * Val(dg1.Item(dg1.ColumnCount - 4, e.RowIndex).Value), 0)), "0.00")
            End If
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
en:
        ...
        If e.ColumnIndex = 8 Then
            ...
            Dim cmd As New SqlCommand
            Dim dr As SqlDataReader
            cmd = New SqlCommand("Select Gbalpcs from tbllotr where lotno='" & dg1.Item(1, e.RowIndex).Value & dg1.Item(2, e.RowIndex).Value & "'", cn)
            dr = cmd.ExecuteReader
            While dr.Read
                If dg1.Item(8, e.RowIndex).Value > Val(dr.Item(0)) Then
                    MessageBox.Show("Enter less value then Gbalpcs value in lotr")
                    TextBox14.Focus()
                    dg1.Focus()
                    dg1.Item(8, e.RowIndex).Value = 0
        ...
        Dim cmd As New SqlCommand
        Dim dr As SqlDataReader
        cmd = New SqlCommand("Select GbalMtr from tbllotr where lotno='" & dg1.Item(1, e.RowIndex).Value & dg1.Item(2, e.RowIndex).Value & "'", cn)
        dr = cmd.ExecuteReader
        While dr.Read

            If dg1.Item(9, e.RowIndex).Value > Val(dr.Item(0)) Then
                MessageBox.Show("Enter less value then Gbalpcs value in lotr")
                    TextBox14.Focus()
                    dg1.Focus()

                    dg1.Item(9, e.RowIndex).Value = 0
...
en:
    ...
    Private Sub Button6_Click(sender As Object, e As EventArgs) Handles Button6.Click
        Try
            ...
            Dim cmd As New SqlCommand
            Dim dr As SqlDataReader
            cmd = New SqlCommand("Select Gbalpcs from tbllotr where lotno='" & dg1.Item(1, currentpos).Value & dg1.Item(2, currentpos).Value & "'", cn)
            dr = cmd.ExecuteReader
            While dr.Read
                If dg1.Item(8, currentpos).Value > Val(dr.Item(0)) Or dg1.Item(8, currentpos).Value = 0 Then
                    MessageBox.Show("Enter less value then Gbalpcs value in lotr")
                    TextBox14.Focus()
                    dg1.Focus()
                    dg1.Item(8, currentpos).Selected = True
                    GoTo en
            ...
            '  MessageBox.Show("Insert")
            cmd = New SqlCommand("Select GbalMtr from tbllotr where lotno='" & dg1.Item(1, currentpos).Value & dg1.Item(2, currentpos).Value & "'", cn)
            dr = cmd.ExecuteReader
            While dr.Read

                If dg1.Item(9, currentpos).Value > Val(dr.Item(0)) Or dg1.Item(9, currentpos).Value = 0 Then
                    MessageBox.Show("Enter less value then Gbalpcs value in lotr")
                    TextBox14.Focus()
                    dg1.Focus()
                    dg1.Item(9, currentpos).Selected = True
                    GoTo en
            ...
            cmd = New SqlCommand("Update tblLotr set Gbalpcs=gbalpcs-" & dg1.Item(8, dg1.CurrentCell.RowIndex).Value & ",gbalmtr=gbalmtr-" & dg1.Item(9, dg1.CurrentCell.RowIndex).Value & " where Lotno='" & dg1.Item(1, dg1.CurrentCell.RowIndex).Value & dg1.Item(2, dg1.CurrentCell.RowIndex).Value & "'", cn)
            cmd.ExecuteNonQuery()
            ...
            dt.Item(0) = tempdataset.Tables("Lotr").Rows.Count + 1
            dt.Item(1) = dg1.Item(8, currentpos).Value
            dt.Item(2) = dg1.Item(9, currentpos).Value
            dt.Item(3) = dg1.Item(12, currentpos).Value
            dt.Item(4) = dg1.Item(13, currentpos).Value
...
en:
    ...
    Private Sub Button9_Click(sender As Object, e As EventArgs) Handles Button9.Click
        Try
            ...
            For I = 0 To tempdataset.Tables("LOTR").Rows.Count - 1
                '  MessageBox.Show("1")
                If tempdataset.Tables("LOTR").Rows(I).Item(5).ToString = dg1.Item(1, dg1.CurrentCell.RowIndex).Value & dg1.Item(2, dg1.CurrentCell.RowIndex).Value And tempdataset.Tables("lotr").Rows(I).Item(0).ToString = dg1.Item(16, dg1.CurrentCell.RowIndex).Value Then
                    li.Items.Add(I)
                    '          Dim gbalmtr As Decimal = 0
                    '            Dim gbalmtr1 As Decimal = 0
                    '           Dim gbalpcs As Decimal = 0
                    '          '  Dim isdesp As String = "f"
                    '         CMD = New SqlCommand("Select gbalmtr from tblLotrt where SrNo=" & tempdataset.Tables("LOTR").Rows(I).Item(0).ToString & " and lotno='" & tempdataset.Tables("LOTR").Rows(I).Item(5).ToString & "'", cn)
                    '        Dim dr As SqlDataReader
                    '       dr = CMD.ExecuteReader
                    '      While dr.Read
                    '   gbalmtr = Val(dr.Item(0).ToString) + Val(tempdataset.Tables("LOTR").Rows(I).Item(2).ToString)
                    '     End While
                    '      dr.Close()
                    ...
                    connect()
                    ' MessageBox.Show(tempdataset.Tables("LOTR").Rows(I).Item(5).ToString)
