...
Public Class frmbill
    ...
    Private Sub frmbill_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            'new editing
            If frmbillfilter.CheckedListBox3.CheckedItems.Count = 0 Then
                Dim billno As String = ""
                Dim i1 As Integer
                For i1 = 0 To frmbillfilter.CheckedListBox3.Items.Count - 1
                    billno = billno & "billno='" & frmbillfilter.CheckedListBox3.Items(i1).ToString & "' or "
                Next
                billno = billno.Substring(0, billno.Length - 4)
                da = New SqlDataAdapter("Select * from salec1 where (" & billno & ")", cn)
                da.Fill(ds, "salec1")
                da = New SqlDataAdapter("Select * from salesc,tblAccount where tblAccount.ACCode=salesc.ACCode and(" & billno & ") and (billdt>='" & frmbillfilter.dat1.Month & "-" & frmbillfilter.dat1.Day & "-" & frmbillfilter.dat1.Year & "' and billdt<='" & frmbillfilter.dat2.Month & "-" & frmbillfilter.dat2.Day & "-" & frmbillfilter.dat2.Year & "')", cn)
                da.Fill(ds, "Datatable3")
                rptmain.SetDataSource(ds)
            Else
                Dim billno As String = ""
                Dim i1 As Integer
                For i1 = 0 To frmbillfilter.CheckedListBox3.CheckedItems.Count - 1
                    billno = billno & "billno='" & frmbillfilter.CheckedListBox3.CheckedItems(i1).ToString & "' or "
                Next
                billno = billno.Substring(0, billno.Length - 4)
                da = New SqlDataAdapter("Select * from salec1 where (" & billno & ")", cn)
                da.Fill(ds, "salec1")
                '   MsgBox("Select * from salesc,tblAccount where tblAccount.ACCode=salesc.ACCode and(" & billno & ") and (billdt>='" & frmbillfilter.dat1.Month & "-" & frmbillfilter.dat1.Day & "-" & frmbillfilter.dat1.Year & "' and billdt<='" & frmbillfilter.dat2.Month & "-" & frmbillfilter.dat2.Day & "-" & frmbillfilter.dat2.Year & "')")
                da = New SqlDataAdapter("Select * from salesc,tblAccount where tblAccount.ACCode=salesc.ACCode and(" & billno & ") and (billdt>'" & frmbillfilter.dat1.Month & "-" & frmbillfilter.dat1.Day & "-" & frmbillfilter.dat1.Year & "' and billdt<'" & frmbillfilter.dat2.Month & "-" & frmbillfilter.dat2.Day & "-" & frmbillfilter.dat2.Year & "')", cn)
                da.Fill(ds, "Datatable3")
                rptmain.SetDataSource(ds)
            ...
            DataGridView1.DataSource = ds.Tables("datatable3")
