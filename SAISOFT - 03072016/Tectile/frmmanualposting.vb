...
Public Class frmmanualposting
        ...
        Try
            ...
            For I = 0 To COUNT - 1
                ...
                While DR.Read
                    ...
                    LEDGDR = 0
                    LEDGCR = Val(DR.Item("netamt")) - partyamt
                    LEDGBAL = LEDGCR - LEDGDR
                    LEDGREMARK = DR.Item("remark")
                    LEDGREF = DR.Item("oubno")
        ...
        Try
            ...
            For I = 0 To COUNT - 1
                ...
                While DR.Read
                    ...
                    LEDGACCODE = DR.Item("accode")
                    LEDGACNAME = DR.Item("name")
                    LEDGDR = Val(DR.Item("netamt")) - partyamt
                    LEDGCR = 0
                    LEDGBAL = LEDGCR - LEDGDR
                    LEDGREMARK = DR.Item("remark")
                    LEDGREF = DR.Item("oubno")
    ...
    Private Sub maxVrNo(ByRef tb As TextBox, ByVal ds As String)
        ...
        End Try
    End Sub
    Private Sub jourentry(ByVal discount As Decimal, ByVal rd As Decimal, ByVal claim As Decimal, ByVal tds As Decimal, ByVal vrno As String, ByVal acname As String, ByVal accode As String, ByVal dat1 As String, jvvrno As String)
        Try
            ...
            If Val(discount) > 0 Then
                ...
                End While
                dr.Close()
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "DR", disacname, disccode, Val(discount), 0.0, vrno, "BKRCPT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, disccode, disacname, Val(discount), 0.0, -(Val(discount)), vrno, "BKRCPT", 0, 0}
                myinsert(b, "tblLedg")
            ...
            If Val(rd) > 0 Then
                ...
                End While
                dr.Close()
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "DR", disacname, disccode, Val(rd), 0.0, vrno, "BKRCPT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, disccode, disacname, Val(rd), 0.0, -(Val(rd)), vrno, "BKRCPT", 0, 0}
                myinsert(b, "tblLedg")
                amt = amt + Val(rd)
                close1()
            End If
            If Val(claim) > 0 Then
                ...
                End While
                dr.Close()
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "DR", disacname, disccode, Val(claim), 0.0, vrno, "BKRCPT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, disccode, disacname, Val(claim), 0.0, -(Val(claim)), vrno, "BKRCPT", 0, 0}
                myinsert(b, "tblLedg")
            ...
            If Val(tds) > 0 Then
                ...
                End While
                dr.Close()
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "DR", disacname, disccode, Val(tds), 0.0, vrno, "BKRCPT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, disccode, disacname, Val(tds), 0.0, -(Val(tds)), vrno, "BKRCPT", 0, 0}
                myinsert(b, "tblLedg")
                amt = amt + Val(tds)
                close1()
            End If
            If amt > 0 Then
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "CR", acname, accode, 0.0, amt, vrno, "BKRCPT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, accode, acname, 0.0, Val(amt), (Val(amt)), vrno, "BKRCPT", 0, 0}
                myinsert(b, "tblLedg")
            End If
        Catch ex As Exception
            MsgBox(ex.ToString)
    ...
    Private Sub BANKR()
        Try
            ...
            CMD.ExecuteNonQuery()
            CMD = New SqlCommand("update salesc set outamt=netamt,ispaid='f'", cn)
            CMD.ExecuteNonQuery()
            CMD = New SqlCommand("Update tblOub set outamt=netamt,ispaid='f' where book='Sales'", cn)
            CMD.ExecuteNonQuery()
            ...
            While DR.Read
                ...
                jvvrno.Items.Add(DR.Item("jvvrno").ToString)
                itcount2 = itcount2 + 1
            End While
            DR.Close()
    ...
    Private Sub updatebillp()
        ...
        End Try
    End Sub
    Private Sub jourentry1(ByVal discount As Decimal, ByVal rd As Decimal, ByVal claim As Decimal, ByVal tds As Decimal, ByVal vrno As String, ByVal acname As String, ByVal accode As String, ByVal dt As String, jvvrno As String)
        Try
            Dim amt As Decimal = 0.0
            ...
            If Val(discount) > 0 Then
                ...
                End While
                dr.Close()
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "CR", disacname, disccode, 0.0, Val(discount), vrno, "BKPYMT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, disccode, disacname, 0.0, Val(discount), (Val(discount)), vrno, "BKPYMT", 0, 0}
                myinsert(b, "tblLedg")
                amt = amt + Val(discount)
                close1()
            End If
            If Val(rd) > 0 Then
                ...
                End While
                dr.Close()
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "CR", disacname, disccode, 0.0, Val(rd), vrno, "BKPYMT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, disccode, disacname, 0.0, Val(rd), (Val(rd)), vrno, "BKPYMT", 0, 0}
                myinsert(b, "tblLedg")
                amt = amt + Val(rd)
                close1()
            End If
            If Val(claim) > 0 Then
                ...
                End While
                dr.Close()
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "CR", disacname, disccode, 0.0, Val(claim), vrno, "BKPYMT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, disccode, disacname, 0.0, Val(claim), (Val(claim)), vrno, "BKPYMT", 0, 0}
                myinsert(b, "tblLedg")
            ...
            If Val(tds) > 0 Then
                ...
                End While
                dr.Close()
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "CR", disacname, disccode, 0.0, Val(tds), vrno, "BKPYMT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, disccode, disacname, 0.0, Val(tds), (Val(tds)), vrno, "BKPYMT", 0, 0}
                myinsert(b, "tblLedg")
                amt = amt + Val(tds)
                close1()
            End If
            If amt > 0 Then
                Dim a() As String = {jvvrno, dat.Month & "-" & dat.Day & "-" & dat.Year, k + 1, "DR", acname, accode, amt, 0.0, vrno, "BKPYMT"}
                myinsert(a, "tblJour")
                Dim b() As String = {jvvrno, "JOUR", dat.Month & "-" & dat.Day & "-" & dat.Year, accode, acname, Val(amt), 0.0, (-Val(amt)), vrno, "BKPYMT", 0, 0}
                myinsert(b, "tblLedg")
            End If
            close1()
        Catch ex As Exception
    ...
    Private Sub autojvr()
        Try
            ...
            While dr.Read
                ...
                jvvrno.Items.Add(dr.Item("jvvrno"))
            End While
            dr.Close()
            Dim i As Integer
            ...
            For i = 0 To vrno.Items.Count - 1
                ...
                End While
                dr.Close()
                jourentry(discount, rd, claim, tds, vrno.Items(i).ToString, acname.Items(i), accode.Items(i), dt.Items(i), jvvrno.Items(i))
                close1()
            Next
            close1()
    ...
    Private Sub autojvp()
        Try
            ...
            For i = 0 To vrno.Items.Count - 1
                ...
                End While
                dr.Close()
                jourentry1(discount, rd, claim, tds, vrno.Items(i).ToString, acname.Items(i), accode.Items(i), dt.Items(i), jvvrno.Items(i))
                close1()
            Next
            close1()
        Catch ex As Exception
