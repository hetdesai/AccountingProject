    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butSave.Click
        Try
            ...
            If Label11.Text = "ADD Mode" Then
                jourentry()
                updatebill()
                Dim a() As String = {txtVrnofirst.Text & txtVrNosec.Text, dat1.Month & "-" & dat1.Day & "-" & dat1.Year, txtBKCode.Text, txtBkName.Text, txtAccode.Text, txtAcName.Text, txtAmount.Text, txtRemark.Text, txtChqno.Text, txtRefBank.Text, "Bank", txtBkName.Text, txtBKCode.Text, 0, " ", 0, "f", dat1.Month & "-" & dat1.Day & "-" & dat1.Year, ComboBox1.Text, txtJvvrno.Text}
...
last:
    ...
    Private Sub jourentry()
        Try
            If Val(frmoutpmt.discount.Text) > 0 Or Val(frmoutpmt.rdcount.Text) > 0 Or Val(frmoutpmt.claimcount.Text) > 0 Or Val(frmoutpmt.tdscount.Text) > 0 Then
    ...
    Private Sub jourentry1()
        Try
            If Val(frmoutpmt.discountcopy.Text) > 0 Or Val(frmoutpmt.rdcountcopy.Text) > 0 Or Val(frmoutpmt.claimcountcopy.Text) > 0 Or Val(frmoutpmt.tdscountcopy.Text) > 0 Then
                Dim amt As Decimal = 0.0
                Dim tb As New TextBox
                maxVrNo(tb, "tblJour")
                tb.Text = acycode & "/" & tb.Text
