        ...
        Try
            ...
            ElseIf check = 2 Then
                ...
                Try
                    connect()
                    Dim cmd As New SqlCommand
                    cmd = New SqlCommand("update sale1 set desgno='" & TextBox2.Text & "' where desgno='" & TextBox3.Text & "'", cn)
                    cmd.ExecuteNonQuery()
                    cmd = New SqlCommand("update salec1 set desgno='" & TextBox2.Text & "' where desgno='" & TextBox3.Text & "'", cn)
                    cmd.ExecuteNonQuery()
                    close1()
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        Try
            If MessageBox.Show("Are you sure want to delete entry", "Verify", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
                ...
                Dim CMD As New SqlCommand
                Dim dr As SqlDataReader
                CMD = New SqlCommand("Select * from sale1 where desgno='" & TextBox2.Text & "'", cn)
                dr = CMD.ExecuteReader
                If dr.HasRows Then
                    MessageBox.Show("You can not delete this design. Because it is present in sale entry")
                    dr.Close()
                    GoTo EN
                End If
                dr.Close()
                CMD = New SqlCommand("Select * from salec1 where desgno='" & TextBox2.Text & "'", cn)
                dr = CMD.ExecuteReader
                If dr.HasRows Then
                    MessageBox.Show("You can not delete this design. Because it is present in sale entry")
                    dr.Close()
