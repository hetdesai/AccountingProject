...
Public Class frmlotbill
    ...
    Private Sub frmlotbill_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmlotbill_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Dim rpt4 As New copyrpt
            da = New SqlDataAdapter("Select * from sale1 order by convert(INT,SUBSTRING(VRNO,4,LEN(VRNO)-3))", cn)
            da.Fill(ds, "sale1")
            Dim billno As String
            Dim i As Integer
            billno = ""
            If frmlotbillselection.ChkBill.CheckedItemsCount = 0 Then
                For i = 0 To frmlotbillselection.ChkBill.ItemsCount - 1
                    billno = billno & "billno='" & frmlotbillselection.ChkBill.getItems(i).ToString & "' or "

                Next
            Else
                For i = 0 To frmlotbillselection.ChkBill.CheckedItemsCount - 1
                    billno = billno & "billno='" & frmlotbillselection.ChkBill.getCheckedItems(i).ToString & "' or "
                Next
            ...
            acname = ""
            If frmlotbillselection.ChkAccount.CheckedItemsCount = 0 Then
                For i = 0 To frmlotbillselection.ChkAccount.ItemsCount - 1
                    acname = acname & "name='" & frmlotbillselection.ChkAccount.getItems(i).ToString & "' or "

                Next
            Else
                For i = 0 To frmlotbillselection.ChkAccount.CheckedItemsCount - 1
                    acname = acname & "name='" & frmlotbillselection.ChkAccount.getCheckedItems(i).ToString & "' or "
                Next

            End If
            acname = acname.Substring(0, acname.Length - 4)
            Dim pgroup As String

            pgroup = ""
            If frmlotbillselection.ChkGroup.CheckedItemsCount = 0 Then
                For i = 0 To frmlotbillselection.ChkGroup.ItemsCount - 1
                    pgroup = pgroup & "pgroup='" & frmlotbillselection.ChkGroup.getItems(i).ToString & "' or "

                Next
            Else
                For i = 0 To frmlotbillselection.ChkGroup.CheckedItemsCount - 1
                    pgroup = pgroup & "pgroup='" & frmlotbillselection.ChkGroup.getCheckedItems(i).ToString & "' or "
                Next

            End If
            pgroup = pgroup.Substring(0, pgroup.Length - 4)
            da = New SqlDataAdapter("select * from sale where (" & billno & ") and (" & acname & ") order by convert(INT,SUBSTRING(VRNO,4,LEN(VRNO)-3))", cn)
            da.Fill(ds, "sale")
            da = New SqlDataAdapter("select * from tblaccount where (" & pgroup & ")", cn)
            da.Fill(ds, "tblaccount")
            da = New SqlDataAdapter("select * from tblLotr", cn)
            da.Fill(ds, "tblLotr")
            da = New SqlDataAdapter("Select * from salet", cn)
            da.Fill(ds, "salet")
            rpt1.SetDataSource(ds)
            rpt2.SetDataSource(ds)
            rpt3.SetDataSource(ds)
            rpt4.SetDataSource(ds)
