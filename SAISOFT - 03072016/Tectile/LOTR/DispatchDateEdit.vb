...
Public Class DispatchDateEdit
    ...
    Private Sub DispatchDateEdit_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Dim billno As String = FRMDISPATCH.dg3.Item(2, FRMDISPATCH.dg3.CurrentRow.Index).Value
            connect()
            Dim cmd As New SqlCommand
            Dim dr As SqlDataReader
            cmd = New SqlCommand("Select * from sale where billno='" & billno & "'", cn)
            dr = cmd.ExecuteReader
            While dr.Read
                ComboBox1.Text = dr.Item("bkname")
                TextBox1.Text = billno
                TextBox2.Text = billno
