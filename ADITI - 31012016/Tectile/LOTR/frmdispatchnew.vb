...
Public Class frmdispatchnew
    ...
    Private Sub closing()
        Try
            ...
            For i = 0 To tempdataset.Tables("lotr").Rows.Count - 1
                '      dr.Close()
                cmd = New SqlCommand("update tblLotr set gbalmtr=gbalmtr+" & tempdataset.Tables("lotr").Rows(i).Item(2).ToString & ", gbalpcs=gbalpcs+" & tempdataset.Tables("Lotr").Rows(i).Item(1) & " where lotno='" & tempdataset.Tables("lotr").Rows(i).Item(5).ToString & "'", cn)
                cmd.ExecuteNonQuery()
                '    MessageBox.Show("a")
             
            Next
            '   MsgBox(ds2.Tables("lotr").Rows.Count)
            For i = 0 To tempdataset2.Tables("lotr").Rows.Count - 1
                cmd = New SqlCommand("update tblLotr set gbalmtr=gbalmtr-" & tempdataset2.Tables("lotr").Rows(i).Item(2).ToString & ", gbalpcs=gbalpcs-" & tempdataset2.Tables("Lotr").Rows(i).Item(1) & " where lotno='" & tempdataset2.Tables("lotr").Rows(i).Item(5).ToString & "'", cn)
                cmd.ExecuteNonQuery()
                '    MessageBox.Show("av")
            Next
            close1()
    ...
    End Sub
    Private Sub frmdispatchnew_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        '   update1()
        closing()
    ...
    Private Sub frmdispatchnew_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmdispatchnew_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try

            connect()
            Dim cmd As New SqlCommand
            Dim dr As SqlDataReader
            cmd = New SqlCommand("Select value from tblSetup where book='LOTR' and Operation='INVOICEREADONLY'", cn)
            dr = cmd.ExecuteReader
            While dr.Read
    ...
    Public Sub createdataset()
        tempdataset.Tables.Add("Lotr")
        tempdataset.Tables("Lotr").Columns.Add("Srno")
        tempdataset.Tables("lotr").Columns.Add("Gpcs")
        tempdataset.Tables("Lotr").Columns.Add("Gmtr")
        tempdataset.Tables("lotr").Columns.Add("Fpcs")
        tempdataset.Tables("Lotr").Columns.Add("Fmtr")
        tempdataset.Tables("Lotr").Columns.Add("Lotno")
        tempdataset2.Tables.Add("Lotr")
        tempdataset2.Tables("Lotr").Columns.Add("Srno")
        tempdataset2.Tables("lotr").Columns.Add("Gpcs")
        tempdataset2.Tables("Lotr").Columns.Add("Gmtr")
        tempdataset2.Tables("lotr").Columns.Add("Fpcs")
        tempdataset2.Tables("Lotr").Columns.Add("Fmtr")
        tempdataset2.Tables("Lotr").Columns.Add("Lotno")
    ...
    Private Sub fillsale1()
        Try
            ...
            While dr.Read
                dg1.Rows.Add()
                dg1.Item("colSrNo", i).Value = dr.Item("Srno")
                dg1.Item("Colloty", i).Value = dr.Item("lotno").ToString.Substring(0, 3)
                dg1.Item("collotno", i).Value = dr.Item("lotno").ToString.Substring(3)
                dg1.Item("colprocess", i).Value = dr.Item("process")
                dg1.Item("colgrayqly", i).Value = dr.Item("itname")
                dg1.Item("colstyle", i).Value = dr.Item("style")
                dg1.Item("colmst", i).Value = dr.Item("mst")
                dg1.Item("colwidth", i).Value = dr.Item("width")
                dg1.Item("colgpcs", i).Value = dr.Item("grpcs")
                dg1.Item("colgmtr", i).Value = dr.Item("grmtr")
                dg1.Item("colgrate", i).Value = dr.Item("grrate")
                dg1.Item("colgval", i).Value = dr.Item("grval")
                dg1.Item("colfpcs", i).Value = dr.Item("pcs")
                dg1.Item("colfmtr", i).Value = dr.Item("qty")
                dg1.Item("colfrate", i).Value = dr.Item("rate")
                dg1.Item("colfamt", i).Value = dr.Item("amount")
                Try
                    dg1.Item("colshortage", i).Value = Val(dr.Item("shortage"))
                Catch ex As Exception

                End Try

                dg1.Item("coltempsr", i).Value = dr.Item("srno")
                i = i + 1
            End While
            dr.Close()
            close1()
...
en:
    ...
    Private Sub dg1_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dg1.CellClick
        If e.ColumnIndex > 3 And e.ColumnIndex < 12 Then
            dg1.CurrentCell = dg1.Item(13, dg1.CurrentCell.RowIndex)
    ...
    Private Sub dg1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dg1.KeyDown
        Try
            If dg1.CurrentCell.ColumnIndex = dg1.ColumnCount - 3 Then
                If e.KeyCode = Keys.Enter Then
                    Button6.Focus()
                    e.Handled = True
                End If
    ...
    Private Sub dg1_RowsAdded(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowsAddedEventArgs) Handles dg1.RowsAdded
        dg1.item("colsrno", e.RowIndex).Value = e.RowIndex + 1
        dg1.item("colloty", e.RowIndex).Value = ComboBox1.Text
        dg1.item("colfpcs", e.RowIndex).Value = 0.0
        dg1.Item("colfmtr", e.RowIndex).Value = 0.0
        dg1.Item("colgmtr", e.RowIndex).Value = 0.0
        dg1.Item("colgpcs", e.RowIndex).Value = 0.0
        dg1.Item("colgval", e.RowIndex).Value = 0.0
        dg1.Item("colgrate", e.RowIndex).Value = 0.0
        dg1.Item("colwidth", e.RowIndex).Value = 0.0
        dg1.Item("Colfamt", e.RowIndex).Value = 0.0
        If e.RowIndex <> 0 Then
            dg1.item("colprocess", e.RowIndex).Value = dg1.item("colprocess", e.RowIndex - 1).Value
            dg1.item("colstyle", e.RowIndex).Value = dg1.item("colstyle", e.RowIndex - 1).Value
            GoTo en
        End If
        Try
            connect()
            ...
            dr = cmd.ExecuteReader
            While dr.Read
                dg1.item("colprocess", 0).Value = dr.Item(0)
                dg1.item("colstyle", 0).Value = dr.Item(1)
            End While
            dr.Close()
            close1()
        Catch ex As Exception
...
en:
    ...
    Public Sub caldispatch()
        ...
        For i = 0 To dg1.Rows.Count - 1
            TextBox14.Text = Val(TextBox14.Text) + dg1.Item("colfamt", i).Value
            TextBox15.Text = Val(TextBox15.Text) + dg1.item("colfmtr", i).Value
            TextBox16.Text = Val(TextBox16.Text) + dg1.item("colfpcs", i).Value
            TextBox17.Text = Val(TextBox17.Text) + dg1.item("colgmtr", i).Value
            TextBox18.Text = Val(TextBox18.Text) + dg1.item("colgpcs", i).Value
        Next
    ...
    Private Sub dg1_CellValidated(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dg1.CellValidated
        ...
        Try
            If e.ColumnIndex = 2 Then
                ...
                Catch ex As Exception
                    If MsgBox("You want to exit from entry", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                        GoTo en2
                    Else
                        dg1.Item("collotno", dg1.CurrentCell.RowIndex).Selected = True
                        GoTo en2
                    End If
                End Try
            End If
            ...
            If lotcheck = 0 Then
                If e.ColumnIndex = 2 Then
                    If Label23.Text = "ADD" Or Label23.Text = "EDIT" Then
                        connect()
                        Dim cmd As New SqlCommand
                        Dim dr As SqlDataReader
                        cmd = New SqlCommand("select * from tbllotr where lotno='" & dg1.Item("colloty", e.RowIndex).Value.ToString & dg1.Item("collotno", e.RowIndex).Value.ToString & "'", cn)
                        dr = cmd.ExecuteReader
                        If dr.HasRows Then
                            dr.Close()
                        Else
                            dr.Close()
                            dg1.Item("collotno", e.RowIndex).Selected = True
                        ...
                        End If
                            ...
                            cmd = New SqlCommand("select gbalmtr,gbalpcs from tbllotr where lotno='" & dg1.Item("colloty", e.RowIndex).Value.ToString & dg1.Item("collotno", e.RowIndex).Value.ToString & "'", cn)
                            dr = cmd.ExecuteReader
                            If dr.HasRows Then

                                While dr.Read
                                If Label23.Text = "ADD" Then
                                    If (Val(dr.Item(0).ToString) = 0 And Val(dr.Item(1).ToString) = 0) Then
                                        dr.Close()
                                        dg1.Item("collotno", e.RowIndex).Selected = True
                            ...
                            End If
                            dr.Close()
                            cmd = New SqlCommand("select * from tbllotr where lotno='" & dg1.Item("colloty", e.RowIndex).Value.ToString & dg1.Item("collotno", e.RowIndex).Value.ToString & "' and acname='" & txtName.Text & "'", cn)
                            dr = cmd.ExecuteReader
                            If dr.HasRows Then
                                dr.Close()
                            Else
                                ...
                                cmd2 = New SqlCommand("select acname from tbllotr where lotno='" & dg1.Item("colloty", e.RowIndex).Value.ToString & dg1.Item("collotno", e.RowIndex).Value.ToString & "'", cn)
                                dr2 = cmd2.ExecuteReader
                                While dr2.Read
                                    acname = dr2.Item(0)
                                End While
...
label:
                                ...
                                Dim i1 As Integer
                                For i1 = 0 To dg2.Rows.Count - 1
                                    If (dg2.Item(1, i1).Value.ToString.ToUpper.Contains(dg1.Item(2, dg1.CurrentRow.Index).Value.ToString.ToUpper)) Then
                                        dg2.Item(1, i1).Selected = True
                                        GoTo l
                                    End If
                                Next
l:
                                ...
                                dg2.Columns(2).Width = 200
                                    ...
                                    lotcheck = 1
                                    'dg2.Focus()
                                    ' dg1.item("colsrno", e.RowIndex).Selected = True
                                    GoTo en
                                Else
                                    dg1.Item("collotno", e.RowIndex).Selected = True
                                    SendKeys.Send("{LEFT}")
                            ...
                            lotno = dg1.Item("colloty", dg1.CurrentCell.RowIndex).Value.ToString & dg1.Item("collotno", dg1.CurrentCell.RowIndex).Value.ToString
                            Try
                                connect()
                                cmd = New SqlCommand("Select item,mst,gbalmtr,gbalpcs from tblLotr where Lotno='" & lotno & "'", cn)
                                dr = cmd.ExecuteReader
                                While dr.Read
                                    dg1.Item("colgrayqly", dg1.CurrentCell.RowIndex).Value = dr.Item(0)
                                    dg1.Item("colmst", dg1.CurrentCell.RowIndex).Value = dr.Item(1)
                                    dg1.Item("Colgpcs", dg1.CurrentCell.RowIndex).Value = dr.Item(3)
                                    dg1.Item("colgmtr", dg1.CurrentCell.RowIndex).Value = dr.Item(2)
                                End While
                                dr.Close()
                                close1()
                                Try
                                    If Label23.Text = "EDIT" Then
                                        Dim countrow As Integer
                                        For countrow = 0 To tempdataset.Tables("lotr").Rows.Count - 1
                                            If tempdataset.Tables("lotr").Rows(countrow).Item(0) = dg1.Item("colsrno", e.RowIndex).Value And tempdataset.Tables("lotr").Rows(countrow).Item(5) = dg1.Item("colloty", e.RowIndex).Value & dg1.Item("collotno", e.RowIndex).Value Then

                                                dg1.Item("colgmtr", e.RowIndex).Value = dg1.Item("colgmtr", e.RowIndex).Value + tempdataset.Tables("lotr").Rows(countrow).Item(2)
                                                dg1.Item("colgpcs", e.RowIndex).Value = dg1.Item("colgpcs", e.RowIndex).Value + tempdataset.Tables("lotr").Rows(countrow).Item(1)
                                            End If
                                        Next
                                    End If
                        ...
                        Else
                            ...
                            Try
                                ...
                                Dim dr As SqlDataReader
                                connect()
                                cmd = New SqlCommand("Select item,mst from tblLotr where Lotno='" & lotno & "'", cn)
                                dr = cmd.ExecuteReader
                                While dr.Read
                                    dg1.Item("colgrayqly", dg1.CurrentCell.RowIndex).Value = dr.Item(0)
                                    dg1.Item("colmst", dg1.CurrentCell.RowIndex).Value = dr.Item(1)
                ...
                If e.ColumnIndex = 14 Then
                    dg1.Item("colFamt", e.RowIndex).Value = Format(Val(Math.Round((dg1.Item("colfrate", e.RowIndex).Value) * Val(dg1.Item("colfmtr", e.RowIndex).Value), 0)), "0.00")
                End If
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
en:
        ...
        End If
        If lotcheck = 1 Then
            '   dg1.item("colloty", e.RowIndex).Selected = True
            ' MsgBox("q")
            ' dg1.Enabled = False
            dg2.Focus()
        End If
        ...
        If e.ColumnIndex = 8 Then
            If Label12.Text.ToUpper = "ADD" Then
                connect()
                Dim cmd As New SqlCommand
                Dim dr As SqlDataReader
                cmd = New SqlCommand("Select Gbalpcs from tbllotr where lotno='" & dg1.Item("colloty", e.RowIndex).Value & dg1.Item("collotno", e.RowIndex).Value & "'", cn)
                dr = cmd.ExecuteReader
                While dr.Read
                    If dg1.Item("colgpcs", e.RowIndex).Value > Val(dr.Item(0)) Then
                        MessageBox.Show("Enter less value then Gbalpcs value in lotr")
                        TextBox14.Focus()
                        dg1.Focus()
                        dg1.Item("colgpcs", e.RowIndex).Value = 0
        ...
        ElseIf e.ColumnIndex = 9 Then
            If Label12.Text.ToUpper = "ADD" Then
                ...
                Dim cmd As New SqlCommand
                Dim dr As SqlDataReader
                cmd = New SqlCommand("Select GbalMtr from tbllotr where lotno='" & dg1.Item("colloty", e.RowIndex).Value & dg1.Item("collotno", e.RowIndex).Value & "'", cn)
                dr = cmd.ExecuteReader
                While dr.Read

                    If dg1.Item("colgmtr", e.RowIndex).Value > Val(dr.Item(0)) Then
                        MessageBox.Show("Enter less value then Gbalpcs value in lotr")
                        TextBox14.Focus()
                        dg1.Focus()

                        dg1.Item("colgmtr", e.RowIndex).Value = 0
        ...
        If e.ColumnIndex > 6 Then
            Try
                '      MessageBox.Show("s")
                If (Val(dg1.CurrentCell.Value) = 0) Then
                    dg1.CurrentCell.Value = 0
                End If
            Catch ex As Exception
                dg1.CurrentCell.Value = 0
            End Try
    ...
    Private Sub dg1_CellEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dg1.CellEnter
        Try
            If e.ColumnIndex = 14 Then
                ...
                Dim dr As SqlDataReader
                connect()
                cmd = New SqlCommand("Select * from RateMst where Pgroup='" & txtGroup.Text & "' and Process='" & dg1.item("colprocess", e.RowIndex).Value & "' and style='" & dg1.item("colstyle", e.RowIndex).Value & "' and acname='" & txtName.Text & "' and mst='" & TextBox4.Text & "' and item='" & dg1.item("colgrayqly", e.RowIndex).Value & "'", cn)
                dr = cmd.ExecuteReader
                '    MsgBox(TextBox9.Text & "-" & dg1.item("colprocess", e.RowIndex).Value & "-" & dg1.item("colstyle", e.RowIndex).Value & "-" & TextBox1.Text & "-" & TextBox4.Text & "-" & dg1.item("colstyle", e.RowIndex).Value)
                If dr.HasRows = True Then
                    '  MsgBox("het")
                    While dr.Read
                        dg1.Item("colfrate", e.RowIndex).Value = dr.Item("Rate")
                    End While
                    dr.Close()
                Else
                    If MsgBox("You want to insert rate for this combination", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                        ...
                        value = InputBox("Rate")
                        Dim a() As String = {txtGroup.Text, txtName.Text, TextBox4.Text, dg1.item("colprocess", e.RowIndex).Value, dg1.item("colstyle", e.RowIndex).Value, dg1.item("colgrayqly", e.RowIndex).Value, value}
                        myinsert(a, "RateMst")
                        MsgBox("Inserted")
                        dg1.Item("colfrate", e.RowIndex).Value = value
                    End If
...
LAST:
    ...
    Private Sub filldataset()
        Try
            ...
            cmd = New SqlCommand("select distinct lotno from sale1 where vrno='" & txtvrno1.Text & "'", cn)
            dr = cmd.ExecuteReader
            While dr.Read
                li.Items.Add(dr.Item(0))
            End While
            dr.Close()
            tempdataset2.Tables("Lotr").Rows.Clear()
            tempdataset.Tables("Lotr").Rows.Clear()
            '   MsgBox(li.Items.Count - 1)
            For i = 0 To li.Items.Count - 1
                '  MsgBox(dg1.item("colloty", i).Value & dg1.item("collotno", i).Value)
                cmd = New SqlCommand("select Grpcs,Grmtr,Pcs,Qty,SrNo from sale1 where lotno='" & li.Items(i).ToString & "' AND VRNO='" & txtvrno1.Text & "'", cn)
                dr = cmd.ExecuteReader
                While dr.Read
                    Dim dt As DataRow = tempdataset2.Tables("lotr").NewRow
                    dt.Item(0) = dr.Item(4)
                    dt.Item(1) = dr.Item(0)
                    dt.Item(2) = dr.Item(1)
                    dt.Item(3) = dr.Item(2)
                    dt.Item(4) = dr.Item(3)
                    dt.Item(5) = li.Items(i).ToString



                    tempdataset2.Tables("lotr").Rows.Add(dt)
                    Dim dt1 As DataRow = tempdataset.Tables("lotr").NewRow
                    dt1.Item(0) = dr.Item(4)
                    dt1.Item(1) = dr.Item(0)
                    dt1.Item(2) = dr.Item(1)
                    dt1.Item(3) = dr.Item(2)
                    dt1.Item(4) = dr.Item(3)
                    dt1.Item(5) = li.Items(i).ToString


                    tempdataset.Tables("lotr").Rows.Add(dt1)
                End While
                dr.Close()
                ' MsgBox("ok")
            Next
    ...
    Private Sub Button8_Click(sender As Object, e As EventArgs) Handles Button8.Click
        ' closing()
        If MessageBox.Show("You want to delete entry", "Verify", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
            currentrow = dg3.CurrentCell.RowIndex
            deletecode()
            clearall()
            MsgBox("Deleted")
...
last:
        ...
        End Try
    End Function
    Private Function checkrow() As Boolean
        Dim i As Integer
        For i = 0 To dg1.RowCount - 1
            If dg1.Item("colfamt", i).Value <> 0 And dg1.Rows(i).DefaultCellStyle.ForeColor = Color.Red Then
            ElseIf dg1.Item("colfamt", i).Value = 0 Then


            Else
                MessageBox.Show("Row " & (i + 1) & " is not saved ")
                Return False
            End If
        Next
        Return True
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        If dg1.RowCount = 0 Then
            MessageBox.Show("Please enter atleast 1 lot")
            GoTo en1
        ...
        If Label22.Text = "ADD" Then
            Try
                ...
                For I = 0 To dg1.RowCount - 1
                    connect()
                    Try
                        If dg1.Item("colfpcs", I).Value = 0 Then
                            GoTo en
                        End If
                    ...
                    End Try
                    CMD = New SqlCommand("select itcode from tblitem where itname='" & dg1.Item("colgrayqly", I).Value.ToString & "'", cn)
                    connect()
                    dr = CMD.ExecuteReader
                    While dr.Read
                        itcode = dr.Item(0)
                    End While
                    dr.Close()
                    CMD = New SqlCommand("select prcode from prmst where process='" & dg1.Item("colprocess", I).Value.ToString & "'", cn)
                    dr = CMD.ExecuteReader
                    While dr.Read
                        prcode = dr.Item(0)
                    End While
                    dr.Close()
                    CMD = New SqlCommand("select mstcode from mst where mst='" & dg1.Item("colmst", I).Value.ToString & "'", cn)
                    dr = CMD.ExecuteReader
                    While dr.Read
                        mstcode = dr.Item(0)
                    End While
                    dr.Close()
                    Dim b() As String = {txtvrno1.Text, txtbkname1.Text, txtregcode1.Text, txtInvoiceNo.Text, dat.Month & "-" & dat.Day & "-" & dat.Year, txtBillNosecond.Text, txtBillNofirst.Text & txtBillNosecond.Text, dat.Month & "-" & dat.Day & "-" & dat.Year, txtName.Text, txtNamecr.Text, dg1.Item("colsrno", I).Value, dg1.Item("colgrayqly", I).Value, itcode, dg1.Item("colfpcs", I).Value, "1", dg1.Item("colfmtr", I).Value, dg1.Item("colfrate", I).Value, "MTR", dg1.Item("colfamt", I).Value, txtAccode.Text, txtAccodecr.Text, prcode, dg1.Item("colprocess", I).Value, dg1.Item("colstyle", I).Value, mstcode, dg1.Item("colmst", I).Value, dg1.Item("colwidth", I).Value, dg1.Item("colgpcs", I).Value, dg1.Item("colgmtr", I).Value, dg1.Item("colgrate", I).Value, dg1.Item("colgval", I).Value, 0, dg1.Item("colloty", I).Value & dg1.Item("collotno", I).Value, Val(dg1.Item("colshortage", I).Value)}
                    myinsert(b, "sale1")
                Next
en:
                tempdataset.Tables("lotr").Rows.Clear()
        ...
        Else
            Try
                ...
                Dim I As Integer
                For I = 0 To dg1.RowCount - 1
                    If Val(dg1.Item("colfamt", I).Value) <> 0 Then
                        connect()
                        CMD = New SqlCommand("select itcode from tblitem where itname='" & dg1.Item("colgrayqly", I).Value.ToString & "'", cn)
                        connect()
                        dr = CMD.ExecuteReader
...
en:
    ...
    Private Sub Button6_Click(sender As Object, e As EventArgs) Handles Button6.Click
        Dim currentpos As Integer = dg1.CurrentCell.RowIndex
        Try
            If dg1.CurrentRow.ReadOnly = True Then
                MessageBox.Show("This row is already saved")
                GoTo en

            End If
            If Label22.Text.ToUpper = "EDIT" Then

                Dim I As Integer
                ' MessageBox.Show(tempdataset.Tables("LOTR").Rows.Count)

                Dim li As New ListBox
                For I = 0 To tempdataset.Tables("LOTR").Rows.Count - 1
                    Try
                        If tempdataset.Tables("LOTR").Rows(I).Item(5).ToString = dg1.Item("colloty", dg1.CurrentCell.RowIndex).Value & dg1.Item("collotno", dg1.CurrentCell.RowIndex).Value And tempdataset.Tables("lotr").Rows(I).Item(0).ToString = dg1.Item("ColSrno", dg1.CurrentCell.RowIndex).Value Then
                            '       MessageBox.Show("s")
                            li.Items.Add(I)
                            connect()

                            Dim cmd1 As New SqlCommand("Update tblLotr set Gbalmtr=gbalmtr+" & Val(tempdataset.Tables("Lotr").Rows(I).Item(2)) & "," & "Gbalpcs=gbalpcs+" & Val(tempdataset.Tables("Lotr").Rows(I).Item(1)) & " where lotno='" & tempdataset.Tables("LOTR").Rows(I).Item(5).ToString & "'", cn)
                    ...
                    For I1 = 0 To li.Items.Count - 1
                        tempdataset.Tables("lotr").Rows.RemoveAt(Val(li.Items(I1).ToString) - I1)
            ...
            Dim cmd As New SqlCommand
            Dim dr As SqlDataReader
            cmd = New SqlCommand("Select Gbalpcs from tbllotr where lotno='" & dg1.Item("colloty", currentpos).Value & dg1.Item("collotno", currentpos).Value & "'", cn)
            dr = cmd.ExecuteReader
            While dr.Read
                If dg1.Item("colgpcs", currentpos).Value > Val(dr.Item(0)) Or dg1.Item("colgpcs", currentpos).Value = 0 Then
                    MessageBox.Show("Enter less value then Gbalpcs value in lotr")
                    TextBox14.Focus()
                    dg1.Focus()
                    dg1.Item("colgpcs", currentpos).Selected = True
                    GoTo en
                End If
            End While
            dr.Close()
            ...
            cmd = New SqlCommand("Select GbalMtr from tbllotr where lotno='" & dg1.Item("colloty", currentpos).Value & dg1.Item("collotno", currentpos).Value & "'", cn)
            dr = cmd.ExecuteReader
            While dr.Read
                If dg1.Item("colgmtr", currentpos).Value > Val(dr.Item(0)) Or dg1.Item("colgmtr", currentpos).Value = 0 Then
                    MessageBox.Show("Enter less value then Gbalmtr value in lotr")
                    TextBox14.Focus()
                    dg1.Focus()
                    dg1.Item("colgmtr", currentpos).Selected = True
                    GoTo en
            ...
            cmd = New SqlCommand("Update tblLotr set Gbalpcs=gbalpcs-" & dg1.Item("colgpcs", dg1.CurrentCell.RowIndex).Value & ",gbalmtr=gbalmtr-" & dg1.Item("colgmtr", dg1.CurrentCell.RowIndex).Value & " where Lotno='" & dg1.Item("colloty", dg1.CurrentCell.RowIndex).Value & dg1.Item("collotno", dg1.CurrentCell.RowIndex).Value & "'", cn)
            cmd.ExecuteNonQuery()
            dg1.Rows(currentpos).ReadOnly = True
            dg1.Rows(currentpos).DefaultCellStyle.ForeColor = Color.Red
            Dim dt As DataRow = tempdataset.Tables("Lotr").NewRow

            dt.Item(0) = dg1.Item("colsrno", currentpos).Value
            dt.Item(1) = dg1.Item("colgpcs", currentpos).Value
            dt.Item(2) = dg1.Item("colgmtr", currentpos).Value
            dt.Item(3) = dg1.Item("colfpcs", currentpos).Value
            dt.Item(4) = dg1.Item("colfmtr", currentpos).Value
            dt.Item(5) = dg1.Item("colloty", currentpos).Value & dg1.Item("collotno", currentpos).Value
            tempdataset.Tables("lotr").Rows.Add(dt)
            '      MessageBox.Show(tempdataset.Tables("lotr").Rows(0).Item(2))
            ...
            If dg1.CurrentCell.RowIndex = dg1.RowCount - 1 Then
                dg1.Rows.Add()
                dg1.Item("colsrno", dg1.CurrentCell.RowIndex + 1).Selected = True
            End If
            dg1.Focus()
        Catch ex As Exception
            MessageBox.Show(ex.ToString)
        End Try
        Try
            dg1.Item("colshortage", currentpos).Value = Format((dg1.Item("colfmtr", currentpos).Value - dg1.Item("colgmtr", currentpos).Value) * 100 / dg1.Item("colgmtr", currentpos).Value, "0.00")
        Catch ex As Exception
            MessageBox.Show(ex.ToString)
        End Try
...
en:
        ...
        Try
            ...
            For I = 0 To tempdataset.Tables("LOTR").Rows.Count - 1
                ...
                If tempdataset.Tables("LOTR").Rows(I).Item(5).ToString = dg1.Item("colloty", dg1.CurrentCell.RowIndex).Value & dg1.Item("collotno", dg1.CurrentCell.RowIndex).Value And tempdataset.Tables("lotr").Rows(I).Item(0).ToString = dg1.Item("colSrNo", dg1.CurrentCell.RowIndex).Value Then
                    ...
                    Dim cmd As New SqlCommand("Update tblLotr set Gbalmtr=gbalmtr+" & Val(tempdataset.Tables("Lotr").Rows(I).Item(2)) & "," & "Gbalpcs=gbalpcs+" & Val(tempdataset.Tables("Lotr").Rows(I).Item(1)) & " where lotno='" & tempdataset.Tables("LOTR").Rows(I).Item(5).ToString & "'", cn)
                    cmd.ExecuteNonQuery()
                ...
                End If
            Next
            Dim innerrowcount As Integer
            For I = 0 To li.Items.Count - 1
                tempdataset.Tables("lotr").Rows.RemoveAt(Val(li.Items(I).ToString) - I)
                For innerrowcount = Val(li.Items(I).ToString) To tempdataset.Tables("Lotr").Rows.Count - 1
                    tempdataset.Tables("Lotr").Rows(innerrowcount).Item(0) = tempdataset.Tables("Lotr").Rows(innerrowcount).Item(0) - 1
                Next
            ...
            For rowcount1 = 0 To dg1.RowCount - 1
                dg1.Item("Colsrno", rowcount1).Value = rowcount1 + 1
            Next
            close1()
    ...
    Private Sub dgacm_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgacm.CellContentClick

    End Sub


    Private Sub dg3_KeyDown(sender As Object, e As KeyEventArgs) Handles dg2.KeyDown
        Try
            If e.KeyCode = Keys.Enter Then

                dg1.Item("collotno", dg1.CurrentCell.RowIndex).Selected = True
                dg1.CurrentCell.Value = dg2.Item(1, dg2.CurrentCell.RowIndex).Value.ToString.Split("/")(1)
                dg1.Item("colloty", dg1.CurrentCell.RowIndex).Value = dg2.Item(1, dg2.CurrentCell.RowIndex).Value.ToString.Split("/")(0) & "/"
                dg2.Visible = False
                dg1.Focus()
            End If
