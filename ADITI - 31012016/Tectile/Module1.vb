...
Module Module1
    ...
    Public Sub connect()
        ...
        Dim s() As String
        s = appSettings.GetValues("ConnectionPath")
        '   MessageBox.Show(s(0))
        Dim connectionstring As String
        connectionstring = s(0).Replace("NewDb", frmcomdis.code & "NewDb")
        '   cn = New SqlConnection("Data Source=(LocalDB)\v11.0;AttachDbFilename=" & Application.StartupPath & "\" & frmcomdis.code & "NewDb.mdf;Integrated Security=True;Connect Timeout=30")
        cn = New SqlConnection(connectionstring)
        ' cn = New SqlConnection("Data Source=LENOVO-PC\SQLEXPRESS;Initial Catalog=D:\WORK\ADITI\TECTILE\BIN\DEBUG\1NEWDB.MDF;User ID=sa;Password=Password123")
    ...
    Private Sub connect1()
        cn1 = New SqlConnection("Data Source=(LocalDB)\v11.0;AttachDbFilename=" & Application.StartupPath & "\" & frmcomdis.code & "NewDb.mdf;Integrated Security=True;Connect Timeout=30")
        cn1.Open()
    End Sub
    Public Sub close1()
        cn.Close()
    ...
    Public Sub myinsert(ByVal a As Array, ByVal ds As String)
        ...
        Catch ex As Exception
            ...
            System.IO.File.AppendAllText("Logs\insert.txt", "ds$" & sql & "$" & DateTime.Now)
        End Try
    ...
    Public Sub myupdate(ByVal ds As String, ByVal a As Array, ByVal key As String, ByVal con As String)
        ...
        Catch ex As Exception
            MsgBox(ex.ToString)
            System.IO.File.AppendAllText("Logs\update.txt", "ds$" & Sql & "$" & DateTime.Now & vbNewLine)
        End Try
    End Sub
...
en:
    ...
    End Sub
    Public Sub drawgradient(ByVal color1 As Color, ByVal color2 As Color, ByRef F As Form)

        Dim objbrush As New Drawing2D.LinearGradientBrush(F.DisplayRectangle, color1, color2, linearGradientMode:=Drawing2D.LinearGradientMode.Vertical)
        Dim objgraphics As Graphics = F.CreateGraphics
        objgraphics.FillRectangle(objbrush, F.DisplayRectangle)
...
en:
    ...
    End Sub
    Public Sub UserAction(username As String, action As String, description As String, formname As String)
        Try
            connect()
            '   cmd = New SqlCommand("insert into tblUserAction values('" & username + "','" & DateTime.Now.Month & "-" & DateTime.Now.Day & "-" & DateTime.Now.Year & ":" & DateTime.Now.Hour & ":" & DateTime.Now.Minute & "','" & formname & "','" & action & "','" & description & "')", cn)
            'cmd.ExecuteNonQuery()
            close1()
